...
function mapBook(book) {
  return {
    ...
    published_at: normalizeStatus(book.status) === "published" ? new Date().toISOString() : null,
...
function createLegacyHtml(post = {}) {
  if (post.contentSections?.length) {
    return post.contentSections
      .map((section) => `<section><h2>${section.heading || ""}</h2>${(section.paragraphs || []).map((paragraph) => `<p>${paragraph}</p>`).join("")}</section>`)
      .join("");
  }
...
function mapBlogPost(post) {
  return {
    ...
    visual: post.visual || {},
    body_html: createLegacyHtml(post),
...
async function main() {
  ...
  const bookColumns = [
    ...
    "display_order", "publication_date", "published_at", "pages", "language", "isbn", "publisher", "formats", "purchase_links", "sample_id", "related_books",
    "related_blog_ids", "seo", "synopsis", "discoveries", "who_this_book_is_for", "reading_time", "favorite_quotes", "editions", "updated_at", "deleted_at",
  ];
  const blogColumns = [
    "id", "slug", "title", "excerpt", "status", "display_order", "reading_time", "published_at", "category", "featured", "related_book_ids", "visual",
    "body_html", "content", "content_sections", "seo_title", "seo_description", "revision_history", "last_edited", "updated_at", "deleted_at",
  ];
